...
abline(v=9, lty="dashed", col="maroon")

```

::: callout-warning
## Rationale for Selecting the Appropriate Value of K

The highest value based on this analysis was k=9, which produced an overall accuracy rate of 95.57%. The initial drop from k=1 to k=4 is disregarded due to the higher likelihood of an outlier affecting accuracy rates with a smaller number of neighbors (smaller value of k). After the leveling off seen at k=6, the value of k with the highest accuracy is used.
:::
...
@tbl-shark-ktab is a table of k-Nearest Neighbor classification success rates. This table was used to select a value of k that best classifies tornado locations based on the proximity of sharks, conditioned on both the tornado and shark being located in the buffer. Based on this analysis, k=9 was used as it has a maximum overall success rate of 95.57%.

@tbl-shark-nnc is a table of classifications for all 8 tornados located within the buffer zone, listed as either "Sharks Present" if sharks were found to be in close proximity to the starting location of the tornado, or "No Sharks Present" if no sharks were found to be close enough. This table shows that every tornado was classified as having "No Sharks Present", with 5 of the 8 tornados having a confidence level of 100%, while the minimum confidence was still 66.67%, well above chance.
