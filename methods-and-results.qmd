...
#| echo: false
#| warning: false
#| fig-cap: "Starting Tornado Positions from 2021-2025"
...
clean.tor.3up <- clean.tor[clean.tor$EF_SCALE >= 3,]

plot(clean.tor.3up$BEGIN_LON, clean.tor.3up$BEGIN_LAT, type="n", 
     xlab="Longitude",
     ylab="Latitude",
     asp=1,
     main="Starting Position of Southeastern U.S. EF3+ Tornados from 2021-2025")
map("state", col="grey", fill=FALSE, add=TRUE)
points(clean.tor.3up$BEGIN_LON, clean.tor.3up$BEGIN_LAT, 
       col=ef.scale[clean.tor.3up$EF_SCALE+1], 
       pch=16)
legend("bottomright", 
...
#| echo: false
#| warning: false
#| fig-cap: "Tornado Trajectories from 2021-2025"
#| label: fig-tornado-traject
#| 
plot(clean.tor$END_LON, clean.tor$END_LAT, type="n", 
     xlab="Longitude",
     ylab="Latitude",
     asp=1,
     main="Trajectories of Southeastern U.S. EF3+ Tornados from 2021-2025")
segments(x0=clean.tor.3up$BEGIN_LON,
         y0=clean.tor.3up$BEGIN_LAT, 
         x1=clean.tor.3up$END_LON,
         y1=clean.tor.3up$END_LAT, 
...
@fig-tornado-location shows all recorded tornados in states that border the Gulf of Mexico[^methods_and_results-1] or the Atlantic Ocean. The color of each dot represents the EF rating of the tornado. In total, 3,116 observations were collected within the area of interest.
...
@fig-tornado-traject shows the trajectories of all tornados from @fig-tornado-location. Most of the tornadoes do not tend to travel far from their original spot in comparison to the size of the U.S., and there is a much higher concentration of tornados moving northeast through states like Alabama and Mississippi.
...
names(shark.species9)[names(shark.species9) == "Var1"] <- "Scientific Name"
names(shark.species9)[names(shark.species9) == "Freq"] <- "Frequency"

shark.species9$`Scientific Name`<-str_to_title(shark.species9$`Scientific Name`)
...
#| fig-height: 6
#| fig-subcap: 
#|   - "Overall Accuracy Plot for Values of K based on the k-Table"
#|   - "Error Rates for Values of K based on the k-Table"
...
error.rate<-1-save.results.cv[,4]
plot(x=save.results.cv[,1],
     y=error.rate, 
     main="Error Rates for Values of K", 
     panel.first=grid(), 
     type="o", 
     pch=16,
     col="black", 
     ...
     ylab="Error Rate (%)",
     ylim=c(0,1))
k.err<-save.results.cv[save.results.cv[,1]>4,][which.min(error.rate[save.results.cv[,1]>4]), 1]
k.errac <- 1-k.accuracy
abline(v=k.err, lty="dashed", col="maroon")
...
boot.shark<-replicate(100, {
  ocean.loc<-st_coordinates(st_sample(sharknado.zone, size=2675))
  train.data<-rbind(shark.loc, ocean.loc)
  as.character(knn(train=train.data,
                   test=test.data,
                   cl=table.labels, 
                   k=9))})

shark.prob<-rowMeans(boot.shark == "Shark_Present")
knn.table<-data.frame(Tornado_Number=1:nrow(tor.zone),
                      Classification=ifelse(shark.prob>0.5, "Shark Present", "No Sharks Present"),
                      Confidence=ifelse(shark.prob>0.5, shark.prob, 1-shark.prob))
knitr::kable(knn.table)

```

@fig-shark-ktab-1 is a plot of accuracy rates for values of K. @fig-shark-ktab-2 is a plot of the error rate associated with each value of K. Based on this analysis, k=`r k.max` was used as it has a maximum overall success rate of `r k.accuracy` with a minimized error rate of `r k.errac`.

@tbl-shark-nnc is a table of classifications for all 8 tornados located within the buffer zone, listed as either "Sharks Present" if sharks were found to be in close proximity to the starting location of the tornado, or "No Sharks Present" if no sharks were found to be close enough. This table used a bootstrapping method with 100 repetitions in order to obtain these results. The table shows all 8 tornados fell under the classification of "No Sharks Present", indicating a low risk of sharks being picked up by the tornadoes that fell within the buffer zone based on their path.

According to author Tom Sherrington, "While it’s not a universal certainty, storms can indeed influence shark distribution and bring them closer to shore due to a complex interplay of environmental factors ... \[it’s\] important to note that not all shark species respond to storms in the same way. Some species, like bull sharks, are known for their tolerance of brackish water and are more likely to be found closer to shore even under normal conditions. Others, like great white sharks, may venture closer to shore in search of larger prey that have been impacted by the storm" [@sherringtonStormsBringSharks2024]. As noted in @tbl-sharks, the most common species of sharks captured were the Atlantic Sharpnose, a species that is sensitive to changes caused by tornados and hurricanes [@crearSensitivitySharkNursery2020]. This means that the calculated figure (`r k.accuracy`) is likely an *underestimation of the true success rate*, given the fact that as a result of environmental conditions, these sharks will move further below the surface and/or away from the tornado.
