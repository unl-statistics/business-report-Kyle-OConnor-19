...
A plot is provided as a visual reference of the locations of these states for your convenience. 
...
The first plot shows all recorded tornados in states that border the Gulf of Mexico^[As the majority of the data for both sharks and tornados was collected before 2025, it is internally referred to as the "Gulf of Mexico" rather than the "Gulf of America" in the raw data files. To avoid confusion and to maintain consistency, this report uses the older term "Gulf of Mexico" to describe the body of water located south of the U.S. to align with the internal raw data.] or the Atlantic Ocean. The color of each dot represents the EF rating of the tornado. 

The second plot shows the trajectories of all tornados from the first plot. Most of the tornadoes do not tend to travel far from their original spot in comparison to the size of the U.S., and there is a much higher concentration of tornados moving northeast through states like Alabama and Mississippi.
...
![Sandbar Shark](sandbar-shark.jpg){style="border: 2px solid black;"}
...
This table shows the frequency of sharks species caught off the Gulf and South Atlantic Coasts between 2021-2025. The shark species with the highest observed frequency was the Rhizopriondon Terranovae, whose common name is the Atlantic Sharpnose Shark, with 1,411 total observations. The next highest was the Carcharhinus Plumbeus, whose common name is the Sandbar Shark, with 48 observations.
...
## Shark-Tornado Spatial Cross-Analysis
...
plot(x = save.results.cv[,1], y = save.results.cv[,4], 
     main = "Cross-Validation Accuracy Rates for Values of K", panel.first = grid(), type = "o", 
     col = "black", xlab = "K", ylab = "Accuracy Percenatge")
legend("bottomright", legend = "Correct overall",
       col = "black", lty = 1, pch = 1, 
       bty = "n", cex = 0.95)
